...
// This page is disc-specific, so only the two disc diagnoses are offered. The
// broader neck-and-back calculator covers whiplash, stenosis, radiculopathy, and
// fractures.
const DISC_OPTIONS: { value: Diagnosis; label: string }[] = [
  { value: 'bulgingDisc', label: 'Bulging disc (contained)' },
  { value: 'herniatedDisc', label: 'Herniated disc (ruptured)' },
]
...
// Promoted into Simple mode: "how much is a 2 herniated disc settlement" is the
// single highest-volume question in this cluster.
const LEVEL_OPTIONS = [
  { value: '1', label: 'One disc' },
  { value: '2', label: 'Two discs' },
  { value: '3', label: 'Three or more discs' },
]
...
// Purpose-built herniated disc calculator. Reuses the tested neck-and-back engine
// (calculateNeckInjury), narrowed to disc injuries, with the number of affected
// discs surfaced up front.
export default function HerniatedDiscForm({ config }: { config: CalculatorConfig }) {
  ...
  function buildFields(): ReportField[] {
    ...
    f.push({ label: 'Disc injury', value: optionLabel(DISC_OPTIONS, values.diagnosis) })
    f.push({ label: 'Treatment', value: optionLabel(TREATMENT_OPTIONS, values.treatment) })
    f.push({ label: 'Discs affected', value: optionLabel(LEVEL_OPTIONS, values.levels) })
      ...
      <section className="lg:col-span-3">
        ...
        >
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            <SelectField label="Bulging or herniated disc?" value={values.diagnosis} onChange={(v) => setField('diagnosis', v as Diagnosis)} options={DISC_OPTIONS} />
            <SelectField label="How many discs are affected?" value={values.levels} onChange={(v) => setField('levels', v)} options={LEVEL_OPTIONS} />
          ...
          <p className="mt-4 text-xs leading-5 text-ink-faint">
            Whether a surgeon operated matters more than which disc was injured. A
            multi-level fusion always counts as at least two discs, because by definition
            it fuses more than one.
          </p>
