...
export default function ExampleEstimates({ examples, emphasis = 'total' }: ExampleEstimatesProps) {
  ...
  return (
    <section className="mt-12 rounded-2xl border border-surface-border bg-surface-muted p-6 sm:p-8">
      ...
      <p className="mt-2 max-w-content text-sm leading-6 text-ink-soft">
        A few realistic scenarios to show how the {ps ? 'pain and suffering' : 'range'} moves
        with injury severity. Each uses the same formula as the calculator above, with no
        fault deducted. Your own numbers will differ.
      </p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        {examples.map((ex) => {
          ...
          const low = ps ? result.painSufferingLow : result.low
          const high = ps ? result.painSufferingHigh : result.high
          return (
            <div
              key={ex.label}
            ...
            >
              ...
              {ps && <p className="mt-1 text-xs text-ink-faint">pain &amp; suffering</p>}
            </div>
          )
        })}
      </div>
